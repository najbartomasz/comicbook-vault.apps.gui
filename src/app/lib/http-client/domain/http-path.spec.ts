...
describe(HttpPath, () => {
    ...
    test('should throw error for path not starting with "/"', () => {
        // Given, When, Then
        expect(() => HttpPath.create('api/v1/resources'))
            .toThrowError('Invalid HTTP path: must start with \'/\' but got \'api/v1/resources\'');
    });

    test('should throw error for path containing double slashes', () => {
        // Given, When, Then
        expect(() => HttpPath.create('/api//v1/resources'))
            .toThrowError('Invalid HTTP path: contains double slashes \'/api//v1/resources\'');
    });

    test('should throw error for path ending with "/"', () => {
        // Given, When, Then
        expect(() => HttpPath.create('/api/v1/resources/'))
            .toThrowError('Invalid HTTP path: must not end with \'/\' but got \'/api/v1/resources/\'');
    });
});
